...
public class Game {
	...
	//  handle question and answer processing
	private String handleQuestionAnswer(int questionIndex, int moneyDisplay) {
	    String playerAnswer = null; // Default to null
	    boolean loop = true;

	    while (loop) {
	        System.out.println("*Question - $" + moneyDisplay + "*");
	        ...
	        if (playerAnswer.equalsIgnoreCase("A") || playerAnswer.equalsIgnoreCase("B")
	                || playerAnswer.equalsIgnoreCase("C") || playerAnswer.equalsIgnoreCase("D")) {
	            System.out.print("Confirm? (Y/N, X to quit) >> ");
	            String confirmation = sc.nextLine().trim();

	            if (confirmation.equalsIgnoreCase("Y")) {
	                loop = false; // Exit loop if confirmed
	...
	private boolean easyRound1() {
		...
		for (int i = 1; i <= questionsPerRound; i++) {
			...
			String playerAnswer = handleQuestionAnswer(questionIndex, moneyDisplay);

	        if (playerAnswer == null) { // Player chose to quit
	            return false;
	        }
